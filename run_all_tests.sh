...
if [ "$START_SERVERS" = "yes" ]; then
    ...
    if [ "$TEST_MATH" = true ]; then
        ...
        CMD_ARGS=(python src/math_server/server.py --transport http --host 127.0.0.1 --port "$MATH_PORT")
    ...
    if [ "$TEST_STATS" = true ]; then
        ...
        CMD_ARGS=(python src/stats_server/server.py --transport http --host 127.0.0.1 --port "$STATS_PORT")
        if [ -n "$CONFIG_FILE" ]; then
            CMD_ARGS+=(--config "$CONFIG_FILE")
        fi
