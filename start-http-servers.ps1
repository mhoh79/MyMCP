...
function Start-Servers {
    ...
    $mathArgs = @("-m", "src.builtin.math_server", "--transport", "http", "--host", "0.0.0.0", "--port", $MathPort, "--config", $ConfigFile)
    if ($DevMode) {
        $mathArgs += "--dev"
    }
    ...
    # Build argument list with optional --dev flag
    $statsArgs = @("-m", "src.builtin.stats_server", "--transport", "http", "--host", "0.0.0.0", "--port", $StatsPort, "--config", $ConfigFile)
    if ($DevMode) {
        $statsArgs += "--dev"
    }
