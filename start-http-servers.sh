...
start_servers() {
    ...
    MATH_CMD="python3 -m src.builtin.math_server --transport http --host 0.0.0.0 --port $MATH_PORT --config \"$CONFIG_FILE\""
    if [ "$DEV_MODE" = true ]; then
    ...
    # Build command with optional --dev flag
    STATS_CMD="python3 -m src.builtin.stats_server --transport http --host 0.0.0.0 --port $STATS_PORT --config \"$CONFIG_FILE\""
    if [ "$DEV_MODE" = true ]; then
